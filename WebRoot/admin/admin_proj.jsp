...
function checkfn(){
	...
	document.getElementById("btnAB").style.display = "";
...
function jump(use){
	if(use==1){
		...
		if(confirm("确定通过"+projectId+"项目的审核吗？")){
			...
			post("/MR_ST/admin/validation.jsp?projectId="+projectId+"&sid="+"<%=sid%>"+"&validation=true",comments);
		
		}		
	}else if(use==2.5){
		if(confirm("确定不通过"+projectId+"项目的审核吗？")){
			var comments = prompt("请输入您的备注，无需备注点击取消","");
			post("/MR_ST/admin/validation.jsp?projectId="+projectId+"&sid="+"<%=sid%>"+"&validation=false",comments);
		
		}
	}else if(use==3){
		window.location.href="/MR_ST/admin/assignment.jsp?projectId="+projectId+"&sid="+"<%=sid%>";
	}else if(use==3.5){
		window.location.href="/MR_ST/admin/re_assignment.jsp?projectId="+projectId+"&sid="+"<%=sid%>";
	}else if(use==4){
		window.location.href="/MR_ST/admin/validationwork.jsp?projectId="+projectId+"&sid="+"<%=sid%>";
	}else if(use==5){
		window.location.href="/MR_ST/admin/view_result.jsp?projectId="+projectId+"&sid="+"<%=sid%>";		
	}else if(use==6){
		...
		if(confirm("确定还原"+projectId+"子项目吗？")){
			var comments = "";
			post("/MR_ST/local/abandon.jsp?projectId="+projectId+"&sid="+"<%=sid%>"+"&re=re",comments);		
		}
	}
...
<div class="mainbody">
	...
	<div class="left-navbar" >
		...
		<div class="navbar">
		<a href="/MR_ST/admin/admin_proj.jsp" style="background-color: rgb(150, 185, 125); font-weight: bold; color: rgb(255, 255, 255);" >项目管理列表</a>
	...
	<div class="right-main-body" >
		...
		<div class="tree">
		    <ul style="padding-left:0px;">
		        <li class="treeli">
		            <span class="treespan"><i class="icon-minus-sign"></i> 新项目</span> (指定项目管理员后进入"进行中的项目"列表)
		            <ul id="newProjects">
		           
					</ul>
	 			</li>		
	 	        
	 	        <br><span id="newProjects-none" style="margin:10px 0px 0px 40px"></span>
	 	        <br>
	 	        <hr></hr>
		        <li class="treeli">
	 	...
	 	String sql="select distinct MAIN_PROJECT_ID"
		+" from project_sub as a, project as b where a.MAIN_PROJECT_ID=b.PROJECT_ID order by b.SUBMIT_DATE DESC";
						...
						<script>
					  		if(<%=allfinishflag==0%>){
								...
								if(<%=existPnamager%>==1){
									...
									var newli = document.getElementById("<%=mainPn%>");
									//document.getElementById("edProjects").appendChild(ingli);
									document.getElementById("newProjects").appendChild(newli);
								}
							}else{
								document.getElementById("<%=btnManager%>").style.display="none";									
		...
		<input id="btnRE" type="button" class="btn" value="撤销废弃" style="float:right;display:none;background-color:#A80000" onclick="jump('re')" >		
		</div>
		...
		if(document.getElementById("newProjects").childElementCount==0){
			document.getElementById("newProjects-none").innerHTML="无";
		}
