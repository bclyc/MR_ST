...
<script>
	...
	function down(id){
		...
		if(filepath=="无文件"){
			...
			var assIdToDL = id;
			window.location.href="/MR_ST/admin/download_work.jsp?projectId="+${param.projectId}+"&assId="+assIdToDL+"&sid="+"<%=sid%>";
	...
	function jump(use){
		if(use=="R"){
			...
			post("/MR_ST/admin/validationF.jsp?assId="+assId+"&sid="+"<%=sid%>"+"&validation=false",comments);	
		}else if(use=="U"){
			
			window.location.href="/MR_ST/admin/upload_result.jsp?projectId="+${param.projectId}+"&sid="+"<%=sid%>";	
		}
